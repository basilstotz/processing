 ...
 */
  ...
  baum= new Tree(Width/2.0, Height-sizeGreen, sizeTree/10, sizeTree/4.0, sizeTree);
...
void draw() {
  ...
  for (int i=0;i<baelle-1;i++) {
    fill(255, 0, 0);
    ellipse(i*sizeBall*1.5+sizeBall*1.5, Height-sizeGreen-sizeBall/2, sizeBall, sizeBall);
  }
  ...
  case 1: //laden
    ...
    strokeCap(ROUND);
    float mx=lerp(sx,mouseX,0.5);
    float my=lerp(sy,mouseY,0.5);
    line(sx, sy, mx,my);
    noStroke();
    fill(0);
    ellipse(sx, sy, sizeBall, sizeBall);
  ...
  case 5:
    ...
    if (explode>0) {
      explode-=3;
      text("Game Over", (Width/2.0)+random(4*sizeText)-2*sizeText, (Height/2.0)+random(4*sizeText)-2*sizeText );
    }
    else {
      zustand=0;
      level=0;
