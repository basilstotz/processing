...
public class TreeMaker {
	...
	protected void processNode(Object arg, Node node) {
		...
		if (type.equals("FieldDeclaration")) {
			...
			if (codeTree != null && currentParent != null) {
				...
				for (Iterator iterator = fd.getVariables().iterator(); iterator
						.hasNext();) {
	...
	/**
	 * Converts a row no to offset. Calls xyToOffset intrnally - xyToOffset(x,0)
	 * 
	 * @param x
	 *            - row no.
	 */
	...
	public String processPDECode2() {
		...
		ArrayList<DefaultMutableTreeNode> tabList = new ArrayList();
		ArrayList<Integer> tabLineOffset = new ArrayList();
		try {
			if (editor == null) {
